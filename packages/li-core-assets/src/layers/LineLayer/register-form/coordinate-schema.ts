      ...
      properties: {
        coordinateField: {
          ...
          properties: {
            ...
            longitude: {
              type: 'string', title: '起点经度', required: true,
              'x-decorator': 'FormItem', 'x-component': 'FieldSelect',
              'x-component-props': { allowClear: true, placeholder: '请选择字段' },
              enum: [...fieldList],
              'x-reactions': [{ dependencies: ['coordinateType'], fulfill: { state: { visible: '{{ $deps[0] === "table" || $deps[0] === "dms" }}', title: '{{ $deps[0] === "dms" ? "起点经度(度分秒)" : "起点经度" }}' } } }],
            },
            latitude: {
              type: 'string', title: '起点纬度', required: true,
              'x-decorator': 'FormItem', 'x-component': 'FieldSelect',
              'x-component-props': { allowClear: true, placeholder: '请选择字段' },
              enum: [...fieldList],
              'x-reactions': [{ dependencies: ['coordinateType'], fulfill: { state: { visible: '{{ $deps[0] === "table" || $deps[0] === "dms" }}', title: '{{ $deps[0] === "dms" ? "起点纬度(度分秒)" : "起点纬度" }}' } } }],
            },
            targetLongitude: {
              type: 'string', title: '终点经度', required: true,
              'x-decorator': 'FormItem', 'x-component': 'FieldSelect',
              'x-component-props': { allowClear: true, placeholder: '请选择字段' },
              enum: [...fieldList],
              'x-reactions': [{ dependencies: ['coordinateType'], fulfill: { state: { visible: '{{ $deps[0] === "table" || $deps[0] === "dms" }}', title: '{{ $deps[0] === "dms" ? "终点经度(度分秒)" : "终点经度" }}' } } }],
            },
            targetLatitude: {
              type: 'string', title: '终点纬度', required: true,
              'x-decorator': 'FormItem', 'x-component': 'FieldSelect',
              'x-component-props': { allowClear: true, placeholder: '请选择字段' },
              enum: [...fieldList],
              'x-reactions': [{ dependencies: ['coordinateType'], fulfill: { state: { visible: '{{ $deps[0] === "table" || $deps[0] === "dms" }}', title: '{{ $deps[0] === "dms" ? "终点纬度(度分秒)" : "终点纬度" }}' } } }],
            },
            geometry: {
